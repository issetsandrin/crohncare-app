...
const props = defineProps({
  crise: { type: Object, default: null }
})
...
function initForm(val) {
  if (val) {
    return {
      data_hora: val.data_hora ? val.data_hora.slice(0, 16) : agora(),
      tags: parseTags(val.sintomas),
      intensidade: val.intensidade || 3,
      duracao_estimada: val.duracao_estimada || '',
      observacoes: val.observacoes || ''
    }
  }
  return { data_hora: agora(), tags: [], intensidade: 3, duracao_estimada: '', observacoes: '' }
}

const form = reactive(initForm(props.crise))
...
<template>
  <form class="crise-form" @submit.prevent="onSubmit">
    <div class="form-group">
      <label class="form-label">Data e Hora</label>
      <input type="datetime-local" v-model="form.data_hora" class="form-input" required />
    </div>
    ...
    <div class="form-group">
      ...
      <input type="text" v-model="form.duracao_estimada" class="form-input" placeholder="Ex: 2 horas" />
    </div>
...
.tag-sugestao {
  padding: 5px 10px;
  border: 1px dashed rgba(229, 115, 115, 0.35);
  border-radius: 6px;
  background: transparent;
